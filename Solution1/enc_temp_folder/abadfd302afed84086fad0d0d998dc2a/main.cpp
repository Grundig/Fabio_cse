...
void testGeneralItemArraySearch() {
	...
	if (testitem.IsLargerThan(&testitem1, &testsort_crit))
	{
		cout << "Minimum item selection is larger than maximum. Min and max items will be swapped. " << endl;
		studentrecord_item temp_item = testitem;
		testitem = testitem1;
		testitem1 = temp_item;
	}
